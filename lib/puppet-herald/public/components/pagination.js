(function(angular){
  ...
  module.factory('PaginationFactory', function() {
    ...
    function Pagination(page, limit, elements, pages) {
      if (page < 1) { throw new TypeError('Value for pagination page is invalid: ' + page); }
      if (limit < 1) { throw new TypeError('Value for pagination limit is invalid: ' + limit); }
      var self = {
        page: parseInt(page),
      ...
      if (elements != null) { self.elements = parseInt(elements); }
      if (pages != null) { self.pages = parseInt(pages); }
      function verifyPage(candidate) {
        var icand = parseInt(candidate);
        if (icand < 1) { throw new TypeError('Value for pagination page is invalid: ' + page); }
        if (self.this.pages() != null && icand > self.this.pages()) { 
          throw new TypeError('Value for pagination page is invalid: ' +
              page + ', max page: ' + self.this.pages());
        }
        return icand;
      }
